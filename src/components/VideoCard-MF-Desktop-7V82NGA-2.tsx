  ...
  if (video.isShort || poster) {
    return (
      <Link href={`/shorts/${video.id}`} className="w-full cursor-pointer group/card block">
        ...
        >
          {!isImageLoaded && !imageError && imageUrl && <Skeleton className="absolute inset-0" />}
          {imageError || !imageUrl ? (
            videoUrlForPreview ? (
              <video
                src={`${videoUrlForPreview}#t=0.1`}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-900 border border-white/10 rounded-[15px]">
                <span className="text-zinc-500 font-medium px-4 text-center truncate w-full text-xs">No preview available</span>
              </div>
            )
          ) : (
            <Image
            ...
            videoUrlForPreview ? (
              <>
                <video
                  ...
                  onLoadedData={() => setIsImageLoaded(true)}
                />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/card:opacity-100 transition-opacity flex flex-col items-center justify-center">
                  <Share2 className="h-12 w-12 text-white/80 mb-2 drop-shadow-lg" />
                  <span className="text-white font-medium px-4 text-center truncate w-full drop-shadow-md">{displayTitle}</span>
                </div>
              </>
            ) : (
          ...
          {video.authorName && video.authorUrl && (
            <div
              className={cn(
                ...
                isPlayerOpen ? "opacity-0 translate-y-4 scale-90 pointer-events-none" : "opacity-0 group-hover/card:opacity-100 group-hover/card:translate-y-0 group-hover/card:scale-100"
              )}
            ...
            >
              <a
                ...
                className="flex items-center gap-3 bg-black/60 hover:bg-black/80 backdrop-blur-xl rounded-full pr-4 pl-1.5 py-1.5 transition-all duration-300 group/author border border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.4)] hover:shadow-[0_0_20px_rgba(168,85,247,0.6)] hover:border-purple-500/50 pointer-events-auto"
              >
