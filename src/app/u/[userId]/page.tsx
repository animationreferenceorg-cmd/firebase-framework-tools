...
import { getUserPortfolioItems, getDatabaseVideosAsPortfolioItems } from '@/lib/portfolio-service';
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setLoading(true);
      try {
        const [profData, portfolioData] = await Promise.all([
          getUserProfile(userId),
          getUserPortfolioItems(userId),
        ]);
        setProfile(profData);
  ...
  if (loading) {
    return (
      <main className="container mx-auto px-4 md:px-6 pt-8 pb-16 min-h-screen text-white">
  ...
  if (!profile && items.length === 0) {
    return (
      <main className="container mx-auto px-4 md:px-6 pt-16 text-center text-white min-h-[60vh] flex flex-col items-center justify-center">
        <h1 className="text-3xl font-bold">Profile Not Found</h1>
        <p className="text-zinc-400 mt-2">The animator profile you are looking for does not exist or has no public work.</p>
      </main>
    );
  }
...
const AVATAR_GLOW_CLASSES: Record<string, string> = {
  ...
  const displayName = profile?.displayName || items[0]?.authorName || 'Animator';
